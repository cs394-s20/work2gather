...
const GoalGrid = ({ goals, invites, user }) => {
  ...
  useEffect(() => {
    ...
    Object.values(goals).map((goal) =>
      goal.confirmed ? unfinished_temp.push(goal) : pending_temp.push(goal)
    );
    Object.values(invites).map((goal) =>
      goal.confirmed ? unfinished_temp.push(goal) : invitelist_temp.push(goal)
    );
    ...
    <React.Fragment>
      <Grid container spacing={3} direction="row" justify="flex-start">
        <Grid item xs={12} className={classes.griditem}>
          <Typography variant="h4">Unfinished Goals</Typography>
        </Grid>
